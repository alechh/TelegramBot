...
try:
    ...
    def del_keyboard(message):  # удаление клавиатуры в телеграме
    ...
    '''Main function of notifications'''
    ...
    def advice_time(message):  # начало установки времени для советов
    ...
    def error_advice_time(message):  # обработка неправильного времени для советов
    ...
    def print_daily(message):  # вывод приоритетов пользователя
    ...
    def start_daily(message):  # начало установки приоритетов
    ...
    def daily(message):  # установка приоритетов
    ...
    def error_daily_time(message):  # обработка неправильного времени для приоритета
        bot.send_message(message.chat.id, "Неверный формат времени ❌")
        bot.send_message(message.chat.id, "Выберите или введите время", reply_markup=keyboards.time_key())
        bot.register_next_step_handler(message, set_daily)
    ...
    def note(message, q=True):  # добавление заметки пользователя
        if (q):
            try:
        ...
        bot.send_message(message.chat.id,
                         "Сохраните как заметку или введите дату напоминания (дд.мм.гггг чч:мм)\n(если напомнить в ближайшие сутки, то только время)",
